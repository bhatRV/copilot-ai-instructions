---
description: "Comprehensive guide for migrating Java Spring Boot applications from 3.x to 4.0, focusing on Gradle Groovy DSL"
applyTo: "**/*.java, **/build.gradle, **/settings.gradle, **/gradle/libs.versions.toml, **/*.properties, **/*.yml, **/*.yaml"
---

# Spring Boot 3.x to 4.0 Migration Guide (Java)

## Project Context

This guide provides comprehensive GitHub Copilot instructions for upgrading Java Spring Boot projects from version 3.x to 4.0, with emphasis on Gradle Groovy DSL and Java-specific considerations.
...
```groovy
ext {
    springBootVersion = '3.5.6' // Latest 3.x before migrating to 4.0
}
```
...
**Migration:**
- Use JUnit 5 (recommended for Java projects)
...
**Migration for Java:**
```java
// RestClient body() can return null - handle appropriately
...
// Or use Objects.requireNonNull() if null is unexpected
String requiredBody = Objects.requireNonNull(
    restClient.get()
        .uri("https://api.example.com/data")
        .retrieve()
        .body(String.class),
...
map.from(source::getMethod).to(destination::setMethod);
// Calls destination.setMethod(null) if source returns null
...
map.from(source::getMethod).always().to(destination::setMethod);
// Always calls destination.setMethod(value), even if null
```
...
**build.gradle (include optionals explicitly):**
```groovy
bootJar {
...
**build.gradle (example observability stack):**
```groovy
dependencies {
...
- [ ] Update `libs.versions.toml` or `build.gradle` with Spring Boot 4.0.0
...
- [ ] Check for new JSpecify / SpotBugs null-safety warnings
...
6. **JSpecify nullability**: New `@Nullable` annotations may trigger new SpotBugs/NullAway warnings
...
10. **Lombok compatibility**: Verify Lombok version is compatible with Java 21+ and Spring Boot 4.0
